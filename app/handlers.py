...
async def register_city(message: Message, state: FSMContext):
    ...
    if number == str(num):
        ...
        await message.answer(
            "Привіт! Вітаємо тебе в боті FivOne. Тут зібрані курси та марафони, які створила команда спеціалістів і які допоможуть тобі опанувати нові знання легко, цікаво та весело! Cпостерігати за своїм прогресом ти зможеш в особистому профілі на сайті.",
            reply_markup=main,
        )
        await message.answer(
        ...
        await message.answer("Номер не співпадає з вказаним при авторизації.")
        await state.clear()
