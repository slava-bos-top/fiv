        ...
        if "None is not in list" in str(v):
            if FREE_ACCESS_MODE:
                await callback.message.answer(
                    text="<b>Зараз йде доопрацювання функції збереження прогресу. Ця функція буде доступна через декілька днів. Коли функція буде додана, Вам прийде лист, де потрібно буде зареєструватись на сайті, після чого Ви отримаєте можливість зберігати прогрес!</b>",
                    parse_mode="HTML"
                )
            else:
                await callback.message.answer(
